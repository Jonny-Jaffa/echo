...
let receptionServerModule = null;
...
function loadReceptionServerModule() {
  if (!receptionServerModule) {
    receptionServerModule = require(path.join(getRoleSourceRoot(RUNTIME_ROLE_RECEPTION), "server.js"));
  }

  return receptionServerModule;
}

function loadRoomServiceModule() {
  if (!roomServiceModulePromise) {
    const roomServiceModuleUrl = pathToFileURL(
      path.join(getRoleSourceRoot(RUNTIME_ROLE_ROOM), "index.js"),
    ).href;
...
async function openRoleExperience() {
  ...
  const child = spawn(process.execPath, app.isPackaged ? [] : [path.join(__dirname, "..")], {
    env: {
      ...process.env,
      PIP_UNIFIED_ROLE_ENTRY: runtimeRole,
...
async function startReceptionRuntime() {
  ...
  const { createReceptionServer } = loadReceptionServerModule();
