...
Echo is a lightweight LAN-based notification system for surgeries and reception desks. This repository currently contains:

- `apps/echo`: future unified Echo bootstrap app for role selection and one-installer migration
...
- `docs/ROLE_PRODUCT_PLAN.md`: migration plan for the future one-installer, two-role `Echo` product

## Current Product Shape

- `Echo Reception` is currently the LAN host/server role
- `Echo Surgery` is currently the room/client role
...
- The agreed next architectural direction is:
  - one packaged `Echo` product / one installer
